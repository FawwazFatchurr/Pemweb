...
// Query data weapon dengan batasan LIMIT dan OFFSET menggunakan PDO
$query = $pdo->prepare("SELECT * FROM weapon LIMIT :limit OFFSET :offset");
...
$total_query = $pdo->query("SELECT COUNT(*) as total FROM weapon");
...
<div class="main">
    ...
    <div class="table-equip">
            <table>
                ...
                <tbody>
                    <?php foreach ($weapons as $weapon): ?>
                        <tr>
                            <td><?= htmlspecialchars($weapon['name'] ?? 'N/A') ?></td>
                            <td>
                                <!-- Edit Button -->
                                <a href="edit_weapon.php?id=<?= htmlspecialchars($weapon['id']) ?>" style="text-decoration: none;">
                                    <button style="border: none; background: none;">
                                        <!-- Edit Icon -->
                                        <i class="fas fa-edit" style="color: blue; font-size: 20px;"></i>
                                    </button>
                                </a>
                            <form method="POST" action="delete_weapon.php" style="display: inline;">
                                    <input type="hidden" name="weapon_id" value="<?= htmlspecialchars($weapon['id']) ?>">
                                    <!-- Tombol dengan ikon trashcan -->
                                    <button type="submit" onclick="return confirm('Are you sure you want to delete this weapon?');" style="border: none; background: none;">
        ...
        <div class="pagination-container">
                    <!-- Tombol Previous -->
                    <?php if ($page > 1): ?>
                        <a href="?page=<?= $page - 1 ?>" class="pagination-button previous">PREV</a>
                    <?php endif; ?>
                    ...
                    <?php if ($page < $total_pages): ?>
                        <a href="?page=<?= $page + 1 ?>" class="pagination-button next">NEXT</a>
                    <?php endif; ?>
                </div>
    </div>
</div>
