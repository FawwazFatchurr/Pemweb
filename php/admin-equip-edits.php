...
// Query data shield dengan batasan LIMIT dan OFFSET menggunakan PDO
$query = $pdo->prepare("SELECT * FROM shield LIMIT :limit OFFSET :offset");
$query->bindParam(':limit', $items_per_page, PDO::PARAM_INT);
$query->bindParam(':offset', $offset, PDO::PARAM_INT);
$query->execute();
$shields = $query->fetchAll(PDO::FETCH_ASSOC);

// Hitung total data untuk membuat tombol navigasi
$total_query = $pdo->query("SELECT COUNT(*) as total FROM shield");
            ...
            <table>
                ...
                <tbody>
                    <?php foreach ($shields as $shield): ?>
                        <tr>
                            <td><?= htmlspecialchars($shield['name'] ?? 'N/A') ?></td>
                            <td>
                                <!-- Edit Button -->
                                <a href="edit_shield.php?id=<?= htmlspecialchars($shield['id']) ?>" style="text-decoration: none;">
                                    <button style="border: none; background: none;">
                                        <!-- Edit Icon -->
                                        <i class="fas fa-edit" style="color: blue; font-size: 20px;"></i>
                                    </button>
                                </a>
                            <form method="POST" action="delete_shield.php" style="display: inline;">
                                    <input type="hidden" name="shield_id" value="<?= htmlspecialchars($shield['id']) ?>">
                                    <!-- Tombol dengan ikon trashcan -->
                                    <button type="submit" onclick="return confirm('Are you sure you want to delete this shield?');" style="border: none; background: none;">
